...
const AiEnvSetupMessage = ({ options, defaultModels, fastModels, style }) => {
  const envs = options?.ai_envs || [];
  const engines = options?.ai_engines || [];
  const defaultEnv = envs.find(e => e.id === options?.ai_default_env) || envs[0];
  const defaultEngine = defaultEnv && engines.find(e => e.type === defaultEnv.type);
  ...
  if (!envs.length) {
    return (
      <NekoMessage variant="danger" style={{ marginTop: 0, marginBottom: 0, ...style }}>
        {formatWithLink(i18n.SETTINGS.AI_ENV_SETUP, i18n.SETTINGS.AI_ENV_SETUP_URL, i18n.SETTINGS.AI_ENV_SETUP_LINK_TEXT)}
      </NekoMessage>
    );
  }

  if (defaultEnv && !defaultEngine) {
    const html = sprintf(i18n.SETTINGS.AI_DEFAULT_TYPE_UNAVAILABLE, `<b>${escapeName(defaultEnv.name)}</b>`);
    return (
  ...
  const defIssue  = modelsList => modelIssue(options?.ai_default_env, options?.ai_default_model, modelsList);
  ...
  if (dIssue || fIssue) {
    ...
    if (fIssue === 'missing')    bits.push(i18n.SETTINGS.AI_FAST_MISSING);
...
const hasAiEnvIssues = (options, defaultModels, fastModels) => {
  ...
  const defaultEnv = envs.find(e => e.id === options?.ai_default_env) || envs[0];
  const defaultEngine = defaultEnv && engines.find(e => e.type === defaultEnv.type);
  if (defaultEnv && !defaultEngine) return true;
  const requiresKey = defaultEngine && Array.isArray(defaultEngine.inputs) && defaultEngine.inputs.includes('apikey');
  if (requiresKey && !(defaultEnv.apikey && defaultEnv.apikey.length > 0)) return true;
  const check = (envId, modelId, list) => {
    if (!envId || !modelId) return true;
...
const retrieveIgnoredPosts = async () => {
  return await nekoFetch(`${apiUrl}/vectors/ignored`, {
    nonce: getRestNonce()
  });
};
...
export { OptionsCheck, AiEnvSetupMessage, hasAiEnvIssues, cleanSections, useModels, toHTML, useLanguages, addFromRemote, downloadAsFile,
