...
const AiBlockContainer = ({ children, type = "", title = "", hint = "", ...rest }) => {
	...
	const toggleSelectFlag = React.useRef(false);

	React.useEffect(() => {
		if (isSelected) {
			toggleSelectFlag.current = true;
		} else {
			toggleSelectFlag.current = false;
		}
	}, [isSelected]);

	const handleClick = () => {
		if (toggleSelectFlag.current) {
			setIsSelected(false);
		} else {
			setIsSelected(true);
		}
	...
	return (
		<BlockContainer className={classes + (isSelected ? " is-selected" : "")} onClick={handleClick} {...rest}>
