...
const useTemplates = (category = 'playground') => {
  ...
  useEffect(() => {
    if (newTemplates) {
      ...
      setTemplate(newTemplates[1]); // Bug: Should be newTemplates[0], but here uses [1], may cause undefined if array has only one element.
    }
  }, [newTemplates]);
  ...
  const isDifferent = useMemo(() => {
    ...
    if (!originalTpl) return false; // Bug: if not found, should handle.
  ...
  const onRenameClick = () => {
    ...
    saveTemplates(newTemplatesArray);
    setTemplate({ ...newTemplatesArray.find((x) => x.id === template.id) });
  };
  ...
  const onDeleteClick = () => {
    ...
    const newTemplatesArray = templates.filter((x) => x.id !== template.id);
    saveTemplates(newTemplatesArray);
    setTemplate({ ...newTemplatesArray[0] }); // Bug: When deleting the first template, sets to next; if array is empty, might set undefined.
  };
      ...
      <div style={{ margin: '0' }}>
        ...
        <ul>
          {templates.map((x) => (
            <li
              className={
                template.id === x.id ? ('active' + (isDifferent && isEdit ? ' modified' : '')) : ''
              }
        ...
        {isEdit && (
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', marginBottom: 5 }}>
              <NekoButton disabled={template.id === 'default'} className="danger" icon="trash" onClick={onDeleteClick} />
              <NekoButton disabled={template.id === 'default'} className="secondary" icon="pencil" onClick={onRenameClick} />
              <NekoButton disabled={!canSave} className="secondary" style={{ flex: 6 }} onClick={onSaveClick}>
                Save
              </NekoButton>
            </div>
  ...
  }, [templates, template, isLoadingTemplates, isDifferent, isEdit, canSave, resetTemplate, onDeleteClick, onRenameClick, onSaveClick, onSaveAsNewClick, toHTML, i18n]);
