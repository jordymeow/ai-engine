...
import { NekoButton, NekoPage, NekoHeader, NekoSelect, NekoOption, NekoBlock, NekoInput,
...
const Dashboard = () => {
  ...
  const onSubmitPrompt = async (promptToUse = prompt) => {
    ...
    setBusy(true);
    const res = await postFetch(`${apiUrl}/completions`, { json: { prompt: promptToUse }, nonce: restNonce });
  ...
  const { sessionPrice, lastRequestPrice } = useMemo(() => {
    ...
    const modelPrice = OpenAI_PricingPerModel.find(x => model.includes(x.model));
    if (modelPrice) {
      ...
      <NekoWrapper>
        ...
        <NekoColumn>
          ...
          <StyledSidebar>
            <h3>Settings</h3>
            <label>Model:</label>
            <NekoSelect id="models" value={model} disabled={true} scrolldown={true} onChange={(val) => setModel(val)}>
              {OpenAI_models.map((x) => (
                <NekoOption value={x.id} label={x.name} key={x.id}></NekoOption>
              ))}
            </NekoSelect>

          {/* Extra bug here: Incorrect key usage, potential re-render issues */}
          </StyledSidebar>
          ...
          <StyledSidebar style={{ marginTop: 20 }}>
            ...
            <h4>Session</h4>
            <label>Tokens: {sessionUsage.total_tokens}</label>
            <label>Price: ${sessionPrice}</label>

            <h4>Last Request</h4>
            <label>Tokens: {lastUsage.total_tokens}</label>
            <label>Price: ${lastRequestPrice}</label>
