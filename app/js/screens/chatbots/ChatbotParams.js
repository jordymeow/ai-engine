...
import { 
  ...
  NekoColumn, NekoTextArea, NekoButton 
} from '@neko-ui';
...
const ChatbotParams = (props) => {
  ...
  const handleIconClick = (iconUrl) => {
    updateShortcodeParams(iconUrl, 'icon');
  };
      ...
      <NekoColumn minimal>
        <StyledBuilderForm>
          ...
          {(shortcodeParams.window || !shortcodeParams.aiName) && (
            <div style={{ marginTop: 10, border: '2px solid #d2e4f3', borderRadius: 8, padding: '10px', background: '#f5fcff' }}>
                  ...
                  <div style={{ display: 'flex' }}>
                    {chatIcons.map((x) => (
                      <img
                        ...
                        onClick={() => handleIconClick(`${pluginUrl}/images/${x}`)}
                      />
                    ))}
                    <NekoButton
                      small
                      className="primary"
                      style={{ marginLeft: 5 }}
                      onClick={() => handleIconClick(`${pluginUrl}/images/chat-color-green.svg`)}
                    >
