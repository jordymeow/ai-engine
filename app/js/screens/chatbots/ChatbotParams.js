...
const ChatbotParams = (props) => {
  ...
  const [previewIcon, setPreviewIcon] = useState(() => {
    return isCustomURL ? chatIcon : `${pluginUrl}/images/${chatIcon}`;
  });

  useEffect(() => {
    if (isCustomURL) {
      setPreviewIcon(chatIcon);
    } else {
      setPreviewIcon(`${pluginUrl}/images/${chatIcon}`);
    }
  }, [chatIcon, isCustomURL]);

  const builtShortcode = useMemo(() => {
    const params = [];
    for (const key in shortcodeParams) {
      if (shortcodeParams[key] === undefined || shortcodeParams[key] === null ||
        key === 'chatId' || key === 'name' || key === 'maxSentences' ||
  ...
  return (<>
    <NekoWrapper>
      <NekoColumn minimal>
        <StyledBuilderForm>
          <NekoCollapsableCategories keepState="chatbotParams" defaultOpen={true}>
                ...
                <div style={{
                  ...
                  <div className="mwai-builder-row" style={{ marginTop: 0 }}>
                    ...
                    <div className="mwai-builder-col" style={{ width: 48, display: 'flex', alignItems: 'end' }}>
                      <img style={{ marginRight: 0, paddingTop: 0 }} width={48} height={48} src={previewIcon} ref={previewIconRef} />
                    </div>
                  </div>
                  {isCustomURL && (
                    <div className="mwai-builder-row">
