...
const ChatbotParams = (props) => {
  ...
  const aiEnvironment = useMemo(() => {
    return aiEnvironments.find(e => e.id === shortcodeParams.envId) || null;
  }, [aiEnvironments, shortcodeParams.envId]);

  const allAssistants = aiEnvironment?.assistants || [];
  const assistant = useMemo(() => {
    return allAssistants.find(e => e.id === shortcodeParams.assistantId) || null;
  ...
  const environment = useMemo(() => {
    return environments.find(e => e.id === shortcodeParams.embeddingsEnvId) || null;
  ...
  useEffect(() => {
    if (environment?.server === 'gcp-starter' && shortcodeParams.embeddingsNamespace) {
      ...
      // fallthrough intentionally omitted, so this effect only runs once per trigger
    }
  ...
  }, [shortcodeParams, environment, aiEnvironment, currentModel, isChat, isAssistant]);
