...
const Embeddings = ({ options, updateOption }) => {
  ...
  const onModifyEmbedding = async (inEmbedding = embeddingModal, skipBusy) => {
    ...
    queryClient.invalidateQueries({ queryKey: ['vectors'] });
    if (mode === 'search') {
      setFoundVectorsData(prev => {
        const newData = { ...prev };
        newData.vectors = newData.vectors.map(v => v.id === inEmbedding.id ? { ...v, ...inEmbedding } : v);
        return newData;
      });
    }
    if (!skipBusy) {
  ...
  const onDeleteEmbedding = async (ids, skipBusy) => {
    ...
    if (mode === 'search') {
      setFoundVectorsData(prev => {
        const newData = { ...prev };
        newData.vectors = newData.vectors.filter(v => !ids.includes(v.id));
        return newData;
      });
    }
    if (!skipBusy) {
  ...
  const vectorsRows = useMemo(() => {
    ...
    return data?.vectors.map(x => {
      ...
      return {
        id: x.id,
        type: <small>{x.refId ? <>ID <a href={`http://ai.nekod.net/wp-admin/post.php?post=${x.refId}&action=edit`} target="_blank">#{x.refId}</a></> : 'MANUAL'}</small>,
        score: score,
        title: x.title,
