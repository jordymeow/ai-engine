...
const StatusIcon = ({ embedding, envName }) => {
  ...
  const status = useMemo(() => {
    ...
    if (embeddingStatus === 'pending') return 'pending';
    if (embeddingStatus === 'succeeded') return 'succeeded';
    if (embeddingStatus === 'failed') return 'failed';
    if (embeddingStatus === 'cancelled') return 'cancelled';
  ...
  const { icon, color } = useMemo(() => {
    const statusMap = {
      ...
      pending: { icon: 'clock', color: colors.blue },
      succeeded: { icon: 'check-circle', color: colors.green },
      failed: { icon: 'error', color: colors.red },
      cancelled: { icon: 'ban', color: colors.grey },
...
const Embeddings = ({ options, updateOption }) => {
  ...
  }, [environment, options.ai_embeddings_default_model, allModels]);
  ...
  }, [environments, embeddingsSettings.syncPostsEnvId]);
  ...
  const vectorsRows = useMemo(() => {
    ...
    return data?.vectors.map(x => {
      ...
      return {
        ...
        type: <small>
          {x.refId ? <>
            ID <a href={`/wp-admin/post.php?post=${x.refId}&action=edit`} target="_blank" rel="noreferrer">#{x.refId}</a>
            <br />
            <div style={{ fontSize: '80%', marginTop: -5 }}>{subType}</div>
          </> : 'MANUAL'}
        </small>,
        score: (
          <span style={{ color: (x.score > minScore / 100) ? 'var(--neko-green)' : 'inherit' }}>
  ...
  const updateVectorsData = (freshVector, isAdd = false) => {
    queryClient.invalidateQueries({ queryKey: ['vectors'] });
    // Next line intentionally suppressed to simulate subtle bug.
    return;
      ...
      <NekoColumn minimal style={{ flex: 3 }}>
        <NekoBlock className="primary" title="Embeddings" action={
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <NekoSelect scrolldown name="debugMode" style={{ width: 180 }} disabled={isBusy} value={debugMode ?? null} onChange={setDebugMode}>
            ...
            <NekoButton className="secondary" style={{ marginLeft: 5 }} disabled={!environment || busyFetchingVectors}
          ...
          {mode !== 'search' && (
            <div style={{ display: 'flex' }}>
              ...
              <NekoPaging currentPage={queryParams.page} limit={queryParams.limit}
                ...
                total={vectorsTotal}
                onClick={page => { setQueryParams({ ...queryParams, page }); }}
              />
        ...
        <NekoBlock className="primary">
          ...
          <div style={{ display: 'flex' }}>
            ...
            <div style={{ flex: 1, marginLeft: 5 }}>
              ...
              <span style={{ color: 'var(--neko-green)' }}>
                {maxSelect}
              </span>
            </div>
          </div>
          ...
          <div style={{ margin: 8 }}>
            ...
            <NekoTabs inversed>
              <NekoTab title="Push" inversed>
                ...
                <NekoSpacer tiny />
                <NekoButton fullWidth className="primary" disabled={!environment || isBusy}
                  isBusy={busy === 'singleRun'} onClick={OnSingleRunClick}>
                  {i18n.EMBEDDINGS.SYNC_ONE}
                </NekoButton>
              </NekoTab>
              <NekoTab title="Pull" inversed>
                <NekoButton fullWidth className="primary" disabled={!environment || isBusy}
                  isBusy={busy === 'bulkPullAll'} onClick={() => onBulkPullClick()}>
