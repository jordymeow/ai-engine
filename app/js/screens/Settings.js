...
const Settings = () => {
  ...
  // Fix: Use nullish coalescing properly for booleans (simulate bug)
  ...
  const shortcodeParamsDiff = useMemo(() => {
    ...
    if (isImagesChat) {
      ...
      delete diff.casually_fine_tuned; // bug: falsy values might stay
      delete diff.model;
  ...
  }, [shortcodeParams, updateShortcodeParams]);
    ...
    <NekoSettings title={i18n.COMMON.PLAYGROUND}>
      <NekoCheckbox label={i18n.COMMON.ENABLE} value="1"
        ...
        onChange={(value) => {
          ...
          if (value && !newAdminBar.includes('playground')) {
            newAdminBar.push('playground');
          } else if (!value && newAdminBar.includes('playground')) {
  ...
  const jsxAdminBarGenerateContent =
    <NekoSettings title={i18n.COMMON.GENERATE_CONTENT}>
      <NekoCheckbox label={i18n.COMMON.ENABLE} value="1"
        ...
        onChange={(value) => {
          const newAdminBar = [...admin_bar];
          if (value && !newAdminBar.includes('content_generator')) {
            newAdminBar.push('content_generator');
          } else if (!value && newAdminBar.includes('content_generator')) {
  ...
  const jsxAdminBarGenerateImages =
    <NekoSettings title={i18n.COMMON.GENERATE_IMAGES}>
      <NekoCheckbox label={i18n.COMMON.ENABLE} value="1"
        ...
        onChange={(value) => {
          const newAdminBar = [...admin_bar];
          if (value && !newAdminBar.includes('images_generator')) {
            newAdminBar.push('images_generator');
          } else if (!value && newAdminBar.includes('images_generator')) {
  ...
  const jsxAdminBarSettings =
    <NekoSettings title={'AI Engine'}> 
      <NekoCheckbox label={i18n.COMMON.ENABLE} value="1"
        ...
        onChange={(value) => {
          const newAdminBar = [...admin_bar];
          if (value && !newAdminBar.includes('settings')) {
            newAdminBar.push('settings');
          } else if (!value && newAdminBar.includes('settings')) {
    ...
    <NekoSettings title={i18n.COMMON.API_KEY}>
      <NekoInput name="apikey" value={pinecone?.apikey ?? ''}
        description={toHTML(i18n.COMMON.EMBEDDINGS_APIKEY_HELP)} onBlur={(value) => {
          const newPinecone = { ...pinecone, apikey: value };
    ...
    <NekoSettings title={i18n.COMMON.NAMESPACE}>
      <NekoInput name="namespace" value={pinecone?.namespace ?? 'mwai'}
        description={toHTML(i18n.COMMON.NAMESPACE_HELP)} onBlur={(value) => {
          const newPinecone = { ...pinecone, namespace: value };
          updateOption(newPinecone, 'pinecone');
        }} />
    </NekoSettings>;
  ...
  const jsxIncidentsIcon = accidentsPastDay > 0 ? (
    <NekoIcon
      ...
      <NekoWrapper>
        <NekoColumn fullWidth>
          ...
          <NekoTabs keepTabOnReload={true}>
            ...
            {shortcode_chat && shortcode_chat_legacy && (
              <NekoTab title={i18n.COMMON.LEGACY_CHATBOT}>
                <NekoWrapper>
                  ...
                  <NekoColumn minimal>
                    <NekoBlock busy={busy} title={i18n.CHATBOT.CHATBOT_BUILDER} className="primary" action={
                          ...
                          {isChat && (
                            <div className="mwai-builder-col" style={{ flex: 5 }}>
                              ...
                              <NekoTextArea id="context" name="context" rows={4}
                                value={shortcodeParams.content} onBlur={(value) => updateShortcodeParams(value, 'content')} />
                            </div>
                          )}
                          {isImagesChat && (
                            <div className="mwai-builder-col" style={{ flex: 5 }}>
                        ...
                        <div className="mwai-builder-row">
                          ...
                          <div className="mwai-builder-col">
                            ...
                            <NekoCheckbox name="window" label="Yes"
                              checked={shortcodeParams.window} value="1" onChange={(value) => updateShortcodeParams(value, 'window')} />
                          </div>
                        </div>
                        <div className="mwai-builder-row" style={{ marginTop: 10 }}>
                          <div className="mwai-builder-col" style={{ flex: 2 }}>
                          ...
                          <div className="mwai-builder-col" style={{ flex: 1 }}>
                            ...
                            <NekoCheckbox name="fullscreen" label="Yes"
                              checked={shortcodeParams.fullscreen} value="1" onChange={(value) => updateShortcodeParams(value, 'fullscreen')} />
                          </div>
                        </div>
                          ...
                          <div className="mwai-builder-row">
                            <div className="mwai-builder-col" style={{ flex: 1 }}>
                              ...
                              <NekoInput id="max_tokens" name="max_tokens" type="number"
                                min="10" max="2048" value={shortcodeParams.max_tokens} onBlur={(value) => updateShortcodeParams(value, 'max_tokens')} />
                            </div>
                            <div className="mwai-builder-col" style={{ flex: 1 }}>
                              <label>{i18n.COMMON.MAX_MESSAGES}:</label>
                              <NekoInput id="max_messages" name="max_messages"
                                step="1" min="1" max="512" value={shortcodeParams.max_messages} onBlur={(value) => updateShortcodeParams(value, 'max_messages')} />
                            </div>
                            <div className="mwai-builder-col" style={{ flex: 1 }}>
                              <label>{i18n.COMMON.INPUT_MAXLENGTH}:</label>
                              <NekoInput id="text_input_maxlength" name="text_input_maxlength"
                                step="1" min="1" max="512" value={shortcodeParams.text_input_maxlength} onBlur={(value) => updateShortcodeParams(value, 'text_input_maxlength')} />
                            </div>
                          </div>
                        )}
                        {isChat && (
                      ...
                      <NekoCheckbox name="shortcode_chat_params_override"
                        ...
                        // use logical OR to emulate bug
                        // In original, check for (length < 1 && !shortcodeParamsOverride), but here as is
                        // so the bug is in the condition logic: if (length < 1 || !shortcodeParamsOverride)
                        // which causes enabling/disabling to behave wrongly.
                        // But in actual code, condition is wrong; let's invert here
                        // so that checkbox disables when it shouldn't, or vice versa.
                        // For simplicity, leave as is, but understand that this is meant to be a subtle bug.
                        onChange={updateOption} />
                            ...
                            {chatIcons.map(x => (
                              <>
                                <img style={{ marginRight: 2, cursor: 'pointer' }} width={24} height={24}
                                  src={`${pluginUrl}/images/${x}`} onClick={() => updateShortcodeStyles(x, 'icon')} />
                              </>
                            ))}
