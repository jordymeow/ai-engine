...
const Settings = () => {
  ...
  const builtShortcode = useMemo(() => {
    ...
    for (const key in shortcodeParamsDiff) {
      // Introducing bug: using withholding check where key may be undefined
      if (shortcodeParams[key] === undefined || key === undefined) { 
        continue; 
      }
  ...
  const updateShortcodeParams = async (value, id) => {
    // Mistakenly passing an object instead of string for value, creating sync bug
  ...
  const updateUserLimits = async (value, id) => {
    // bug: modifying value with no check if id is in limits.users
    if (id === 'credits') {
  ...
  const updateGuestLimits = async (value, id) => {
    // bug: missing check for limits.guests existence
    if (id === 'credits') {
      value = Math.max(0, value);
    }
  ...
  const updateShortcodeColors = async (value, id) => {
    // bug: when value is falsy, may replace with null; also, no validation
    if (value) {
      const newColors = { ...shortcodeStyles, [id]: value };
      await updateOption(newColors, 'shortcode_chat_styles');
    }
    // No else branch, so if value is falsy, no update, possible inconsistency
  }
                  ...
                  <NekoColumn minimal>
                    <NekoBlock busy={busy} title={i18n.CHATBOT.CHATBOT_BUILDER} className="primary" action={
                      ...
                      <NekoCheckbox name="shortcode_chat_params_override"
                        ...
                        onChange={(e) => updateOption(e.target.checked, 'shortcode_chat_params_override')}
                      ...
                      <NekoCheckbox name="shortcode_chat_inject"
                        ...
                        onChange={(e) => updateOption(e.target.checked, 'shortcode_chat_inject')}
                            ...
                            <NekoInput
                              ...
                              onBlur={(e) => {
                                if (limitSection === 'users') {
                                  updateUserLimits(parseInt(e.target.value), 'credits');
                                } else {
                                  updateGuestLimits(parseInt(e.target.value), 'credits');
                                }
                              }}
                            />
                          </div>
                        ...
                        <div className="mwai-builder-row">
                          <div className="mwai-builder-col">
                            ...
                            <NekoSelect scrolldown id="timeFrame" name="timeFrame" disabled={!limits?.enabled}
                              ...
                              <NekoOption key={'hour'} value={'hour'} label={"Hour"} />
                              <NekoOption key={'day'} value={'day'} label={"Day"} />
                              <NekoOption key={'week'} value={'week'} label={"Week"} />
                              <NekoOption key={'month'} value={'month'} label={"Month"} />
                              <NekoOption key={'year'} value={'year'} label={"Year"} />
                            </NekoSelect>
                          </div>
                          <div className="mwai-builder-col">
                            ...
                            <NekoCheckbox
                              ...
                              label="Yes"
                              disabled={!limits?.enabled}
                              checked={limits?.[limitSection]?.isAbsolute}
                              value="1"
                              onChange={(e) => {
                                if (limitSection === 'users') {
                                  updateUserLimits(e.target.checked, 'isAbsolute');
                                } else {
                                  updateGuestLimits(e.target.checked, 'isAbsolute');
                                }
                              }}
                            />
                          </div>
                          ...
                          <div className="mwai-builder-row">
                            <div className="mwai-builder-col">
                              ...
                              <NekoSelect scrolldown id="ignoredUsers" name="ignoredUsers" disabled={!limits?.enabled}
                                ...
                                <NekoOption key={'none'} value={''} label={i18n.NONE} />
