...
const Settings = () => {
  ...
  useEffect(() => {
    if (performChecksRanRef.current === true) {
      return;
    }
  ...
  useEffect(() => {
    sessionStorage.setItem('mwai_settings_section', settingsSection);
  }, [settingsSection]);
      ...
      {(() => {
        ...
        if (isFixed && dimensionsArray.length === 1) {
          ...
          if (options?.ai_embeddings_default_dimensions !== fixedDim && options?.ai_embeddings_default_dimensions !== String(fixedDim)) {
            setTimeout(() => updateOption(fixedDim, 'ai_embeddings_default_dimensions'), 50);
          }
          return (
            <NekoInput
