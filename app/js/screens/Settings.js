...
const Settings = () => {
  ...
  const refreshOptions = async () => {
    ...
    try {
      ...
      if (err.message) {
        setError(<> {/* Removed comment for clarity */} 
  ...
  const jsxPineconeNamespace =
    <NekoSettings title={i18n.COMMON.NAMESPACES}>
      <NekoInput isCommaSeparatedArray name="namespaces" value={pinecone?.namespaces}
  ...
  const jsxIncidentsIcon = incidentsFiltered > 0 ? <NekoIcon
      ...
      <NekoWrapper>
        <NekoColumn fullWidth>
          ...
          <NekoTabs keepTabOnReload={true}>
            ...
            {shortcode_chat && <NekoTab title={<>{i18n.COMMON.CHATBOTS}</>}><Chatbots options={options} updateOption={updateOption} busy={busy} /></NekoTab>}
            {shortcode_chat && shortcode_chat_discussions && <NekoTab title={i18n.COMMON.DISCUSSIONS}><Discussions /></NekoTab>}
            {module_statistics && <NekoTab title={i18n.COMMON.QUERIES}><Statistics options={options} updateOption={updateOption} busy={busy} /></NekoTab>}
            {module_embeddings && <NekoTab title={i18n.COMMON.EMBEDDINGS}><Embeddings options={options} updateOption={updateOption} /></NekoTab>}
            {module_finetunes && <NekoTab title={i18n.COMMON.FINETUNES}><FineTuning options={options} updateOption={updateOption} refreshOptions={refreshOptions} /></NekoTab>}
            {module_moderation && <NekoTab title={i18n.COMMON.MODERATION}><Moderation options={options} updateOption={updateOption} busy={busy} /></NekoTab>}
            {module_audio && <NekoTab title={i18n.COMMON.AUDIO_TAB}><Audio options={options} updateOption={updateOption} /></NekoTab>}
              ...
              <NekoWrapper>
                <NekoColumn minimal>
                  <NekoBlock busy={busy} title={i18n.COMMON.OPENAI} className="primary">
                    {jsxOpenAiService}
                    {openai_service === 'openai' && <> {jsxOpenAiApiKey} </>}
                ...
                <NekoColumn minimal>
                  ...
                  {module_statistics && <NekoBlock busy={busy} title={i18n.COMMON.STATISTICS} className="primary">{jsxStatisticsData}</NekoBlock>}
                  ...
                  <NekoColumn minimal>
                    <NekoBlock busy={busy} title={i18n.CHATBOT.CHATBOT_BUILDER} className="primary" action={
                      ...
                      <NekoCheckbox name="shortcode_chat_params_override"
                        ...
                        value="1" checked={shortcodeParamsOverride} onChange={updateOption}
                        />
                      <NekoCheckbox name="shortcode_chat_inject"
                        label={i18n.SETTINGS.INJECT_DEFAULT_CHATBOT}
                        value="1" checked={shortcodeChatInject} onChange={updateOption} />
                    </NekoBlock>
                  </NekoColumn>
                </NekoWrapper>
              </NekoTab>
            )}
            {module_devtools && <NekoTab title={i18n.COMMON.DEV_TOOLS}><DevToolsTab options={options} setOptions={setOptions} /></NekoTab>}
