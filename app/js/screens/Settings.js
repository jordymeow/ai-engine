      ...
      Object.keys(openai_usage).forEach((month) => {
        ...
        Object.keys(monthUsage).forEach((model) => {
          ...
          if (model === 'dall-e' ) {
            ...
            // Bug: redeclaring price with const inside here (shadowing outer), but later using var price outside
            let price = modelUsage.images * modelOptionPrice.price;
      ...
      <NekoWrapper>
        ...
        <NekoColumn full>
          ...
          <NekoTabs keepTabOnReload={true}>
            ...
            <NekoTab key="openai-status" title={<>OpenAI Status{accidentsPastDay > 0 ? <>&nbsp;⚠️</> : null}</>}>
