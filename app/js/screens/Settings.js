...
const Settings = () => {
  ...
  const updateUserLimits = async (value, id) => {
    ...
    const newLimitsUsers = { ...limits?.users, [id]: value };
    const newLimits = { ...limits, users: newLimitsUsers };
  ...
  const updateGuestLimits = async (value, id) => {
    ...
    const newLimitsGuests = { ...limits?.guests, [id]: value };
    const newLimits = { ...limits, guests: newLimitsGuests };
    await updateOption(newLimits, 'limits');
  }
      ...
      <NekoWrapper>
        ...
        <NekoColumn fullWidth>
          ...
          <NekoTabs keepTabOnReload={true}>
            ...
            {shortcode_chat && <NekoTab title={<>{i18n.COMMON.CHATBOTS}<small
              style={{ marginLeft: 4, marginTop: -18, marginRight: -8, fontSize: 9 }}>BETA</small></>}>
              <Chatbots options={options} updateOption={updateOption} busy={busy} />
            </NekoTab>}

            {shortcode_chat && <NekoTab title={<>{i18n.COMMON.CHATBOT}<small
              style={{ marginLeft: 4, marginTop: -18, marginRight: -8, fontSize: 9 }}>LEGACY</small></>}>
              <NekoWrapper>
